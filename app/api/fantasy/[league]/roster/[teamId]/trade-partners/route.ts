...
export async function GET(
  ...
  const response = await fetch(
    `${BACKEND}/api/fantasy/${league}/roster/${encodeURIComponent(access.membership.fantrax_team_id!)}/trade-partners?${incoming}`,
    { cache: "no-store", headers: access.identityHeaders },
  ).catch(() => null);
  if (!response) {
    return Response.json({ error: "Trade partner service unavailable" }, { status: 502 });
  }
