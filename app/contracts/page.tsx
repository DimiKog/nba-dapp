...
export default async function ContractsPage() {
  const players = await fetchTopContracts(50);

  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Top Contracts</h1>
        <p className="mt-1 text-slate-500 dark:text-slate-400">2026-27 Season · Top 50</p>
      </div>
            ...
            {players.map((p) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <Link href={`/players/${p.id}`} className="flex items-center gap-3 group">
                      <div className="relative h-9 w-9 shrink-0 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                        {photo ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center text-sm text-slate-400">{p.name[0]}</div>
                        )}
                      </div>
                      <div>
                        <p className="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{p.name}</p>
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-blue-700 dark:text-blue-400 tabular-nums">{p.contract["2026-27"] ?? "—"}</td>
                  <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-400 tabular-nums hidden md:table-cell">{p.contract["2027-28"] ?? "—"}</td>
                  <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-400 tabular-nums hidden md:table-cell">{p.contract["2028-29"] ?? "—"}</td>
                </tr>
              );
            })}
          </tbody>
